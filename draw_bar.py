...
def draw_all_classes(new_name,sma,mid,lar):
    # draw all classes
    name = []
    num = 0
    for i in new_name:
        ...
        for j in range(num):
            new_one += "\n"
        name.append(new_one + i)
        num += 1
        if(num == 6):
            num = 0
    L = [0]*len(name)
    plt.bar(name,sma,bottom = L,label = "small")
    L = [L[i] + sma[i] for i in range(len(name))]
    plt.bar(name,mid,bottom = L,label = "middle")
    L = [L[i] + mid[i] for i in range(len(name))]
    plt.bar(name,lar,bottom = L,label = "large")
...
def draw_classes_sml(new_name,sma,mid,lar):    
    name = []
    num = 0
    for i in new_name:
        print(i)
...
if __name__ == "__main__":
    data ,result  = read_data("voc_result.txt","VOC_classes.txt")
    # data ,result  = read_data("COCO_result.txt","COCO_classes.txt")
    ...
    write_to_file("VOC_sta.csv",result)
